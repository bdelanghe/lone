...
This document defines the Beads work-graph model used to structure and execute
workflows ("molecules").
...
- Execution order is determined only by dependency edges, not naming or list
  order.
...
- A molecule is not a separate storage type. It is an execution view over epic +
  dependencies.
...
- Always pull work from `bd ready`; never assume an issue is runnable from title
  or position.
...
- Use requirement language when adding edges: "B needs A", then
  `bd dep add B A`.
...
- Most teams can stay on issues + epics + dependencies. Add phases only when
...
Formulas (optional) -> Protos (optional) -> Molecules -> Epics -> Issues
(storage)
